...
public interface IAccountDao {
   ...
   void  update1ForRole(AccountRole accountRole);
    //更新密码
    @Update("<script>UPDATE sys_account set password=#{password},salt=#{salt}" +
            "WHERE id=#{id}</script>")
    int updateForPassword(Account account);
    @Select("<script>select count(USERNAME) from sys_account where USERNAME=#{username}</script>")
    int checkAccountName(Account account);
    @Select("<script>SELECT * FROM sys_account where ID=#{id}</script>")
